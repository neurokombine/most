...
class Router:
    ...
    def handle(self, incoming: Incoming, receiver=None) -> list[str]:
        ...
        if getattr(incoming, "group", False):
            # Мост — личный разговор. Бот школы состоит в чатах учеников, и
            # каждое слово оттуда долетает и до моста: отвечать там нельзя, а
            # писать это в журнал стуков — значит показать человеку список
            # чужих чатов вместо ответа на вопрос «кто ко мне стучался».
            self.store.note_group(incoming.channel, incoming.chat_id,
