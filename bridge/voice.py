...
def vernut_pamyat(loader=None) -> bool:
    """Вернуть освобождённую память операционной системе.

    Одного сборщика мусора мало. Питон отдаёт веса модели своему
    распределителю, а тот держит их про запас и ядру не возвращает: замер на
    сервере 15.09 — после `gc.collect()` процесс весил 423 МБ вместо 528, то
    есть вернулась пятая часть. `malloc_trim` отдаёт остальное: 160 МБ.

    Вызов есть только в glibc (Linux). На макбуке и в контейнерах на musl его
    нет — тогда просто ничего не делаем, это не беда.
    """
    loader = loader or (lambda: __import__("ctypes").CDLL("libc.so.6"))
    try:
        libc = loader()
        libc.malloc_trim(0)
...
class WhisperTranscriber(Transcriber):
    ...
    мост тяжелеет с 37 МБ до 530 (замер на сервере 15.09), а на машине из
    программы это половина свободной памяти. Поэтому после долгого простоя
    модель отпускается — обратно она поднимается около четырёх секунд.
    """
    ...
    def release(self) -> bool:
        ...
        не выгнать. Уходят веса модели, а это основная её часть: замер на
        сервере 15.09 — 528 МБ до, 160 МБ после.
        """
